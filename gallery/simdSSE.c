...
static void AddDot4x4SIMD( int, double *, int, double *, int, double *, int );
    ...
    for ( j=0; j<n; j+=4 ){        /* Loop over the columns of C, unrolled by 4 */
        for ( i=0; i<m; i+=4 ){        /* Loop over the rows of C */
           ...
           one routine (four inner products) */
            ...
            AddDot4x4SIMD( k, &A( i,0 ), lda, &B( 0,j ), ldb, &C( i,j ), ldc );
        }
    }
}
...
static void AddDot4x4SIMD( int k, double *a, int lda,  double *b, int ldb, double *c, int ldc )
{
